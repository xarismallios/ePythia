        ...
        return (
          <div className="max-w-4xl mx-auto px-6 py-10 animate-fade-in space-y-6">
            ...
            <div className={`rounded-2xl p-8 border shadow-sm overflow-hidden relative ${pCfg ? `bg-gradient-to-br ${pCfg.bg} border-${pCfg.border}` : 'bg-surface-container-lowest border-outline-variant/10'}`}>
              <div className="flex items-center gap-5 mb-6">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-label font-semibold uppercase tracking-widest text-on-surface-variant mb-1">{t.dashboardTitle}</p>
                  <h2 className="text-2xl font-headline font-bold text-on-surface">{t.dashboardGreeting}, {profile.firstName}!</h2>
              ...
              {levelInfo.nextXP ? (
                ...
                <div className="p-3 rounded-xl bg-secondary-container/30 border border-secondary/20 text-center">
                  <p className="text-secondary font-label font-bold">{t.dashboardMaxLevel}</p>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { val: profile.sessions.length, label: t.dashboardStats[0], color: 'text-primary', icon: '🔍' },
                { val: profile.totalXP, label: t.dashboardStats[1], color: 'text-secondary', icon: '⚡' },
                { val: profile.badges.length, label: t.dashboardStats[2], color: 'text-primary', icon: '🏅' },
                { val: questDone, label: t.dashboardStats[3], color: 'text-secondary', icon: '🎯' },
            ...
            {latestPersona && pCfg && PIcon && (
              <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 shadow-sm overflow-hidden">
                <div className={`bg-gradient-to-br ${pCfg.bg} p-6 border-b border-outline-variant/10`}>
                  <p className="text-xs font-label font-semibold uppercase tracking-widest text-on-surface-variant mb-3">{t.latestProfileLabel}</p>
              ...
              <div className="bg-surface-container-lowest rounded-xl p-8 border border-outline-variant/10 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    ...
                    <p className="text-on-surface-variant text-sm mt-0.5">{t.questBoardSubtitle}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-2xl font-bold text-primary">{questPct}%</span>
                    <p className="text-xs text-on-surface-variant">{questDone}/{questSteps.length} {t.questsOf}</p>
                  </div>
                </div>
                ...
                {questPct === 100 && (
                  <div className="mt-5 p-4 rounded-xl bg-secondary-container/30 border border-secondary/20 text-center animate-fade-in">
                    <p className="text-secondary font-label font-bold text-lg">{t.questCompleteMsg || '🎉 Όλα τα quests ολοκληρώθηκαν! +50 bonus XP'}</p>
                  </div>
                )}
              </div>
            )}
                ...
                {BADGE_ORDER.map(badgeId => {
                  ...
                  return (
                    <div key={badgeId} className={`p-4 rounded-xl border text-center transition-all ${
                      ...
                      {!earned && <p className="text-xs text-outline mt-2">{t.badgeLocked || '🔒 Κλειδωμένο'}</p>}
                    </div>
                  );
                })}
              </div>
            ...
            {profile.sessions.length > 0 && (
              <div className="bg-surface-container-lowest rounded-xl p-8 border border-outline-variant/10 shadow-sm">
                <h3 className="text-xl font-headline font-bold text-on-surface mb-6">📋 {t.sessionHistoryTitle}</h3>
                <div className="space-y-3">
                    ...
                    return (
                      <div key={s.id} className="flex items-center gap-4 p-4 rounded-xl bg-surface-container border border-outline-variant/20">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-on-surface-variant">{new Date(s.date).toLocaleDateString(t.dateLocale || 'el-GR')} • {typeLabels[s.userType]}</p>
                        </div>
                        {s.rating > 0 && (
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center pb-8">
              <button onClick={resetApp}
                ...
                <Sparkles className="w-4 h-4" />{t.newAnalysis || 'Νέα Ανάλυση'}
              </button>
              <button onClick={() => setStep('welcome')}
                ...
                <ArrowLeft className="w-4 h-4" />{t.homeBtn || 'Αρχική'}
              </button>
            </div>
          </div>
        );
