...
export default function EPythia() {
  ...
  const [employeeSector, setEmployeeSector] = useState(''); // 'public' or 'private'
  const [highschoolType, setHighschoolType] = useState(''); // 'epal' or 'general'
...
`.trim();
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1. Σώσε τον lead πρώτα (χωρίς αποτελέσματα ακόμα)
      ...
      // 2. Πάρε τις συστάσεις από το LLM
      ...
      // 3. Ενημέρωσε το lead με τα αποτελέσματα
      await fetch('/.netlify/functions/save-lead', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      }).catch(err => console.error('Lead update error:', err));
  ...
  const isFormComplete = () => {
    const currentQuestions = userType === 'employee' 
      ? (employeeSector === 'public' ? questions.employee_public : questions.employee_private)
      : questions[userType];
            ...
            <div className="max-w-4xl mx-auto bg-gradient-to-br from-slate-800/50 to-slate-800/20 rounded-2xl p-10 border border-slate-700/50 backdrop-blur-sm">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
                ...
                <div className="md:col-span-2">
                  ...
                  <p className="text-slate-300 mb-4">
                    Μετά την ανάλυση AI, θα σε συνδέσουμε με έναν <span className="font-semibold">εξειδικευμένο σύμβουλο καριέρας</span> που έχει ήδη ζήσει το ίδιο path που σκέφτεσαι.
                  </p>
                  <div className="space-y-3 mb-6">
                    ...
                    <div className="flex items-start gap-3">
                      <Check className="w-5 h-5 text-emerald-400 mt-0.5 flex-shrink-0" />
                      <span className="text-sm text-slate-300">Πρακτικές και εφαρμόσιμες συμβουλές</span>
                    </div>
                  </div>
                  <a 
                    href="mailto:pythiacontact@gmail.com?subject=Ενδιαφέρομαι για Coaching e-Pythia"
              ...
              <div className="grid grid-cols-3 gap-4 mt-8 pt-8 border-t border-slate-700">
                <div className="text-center">
                  ...
                  <div className="text-xs text-slate-400">Εξειδικευμένοι Σύμβουλοι</div>
                </div>
                <div className="text-center">
